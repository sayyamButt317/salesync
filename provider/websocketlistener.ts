...
export default function WebSocketListener() {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      setConnectionStatus(attempt === 0 ? "connecting" : "connecting");
          ...
          switch (type) {
            case "whatsapp.message": {
              ...
              if (!toastQueueRef.current[message.id]) {
                ...
                toast.message(message.username || "New WhatsApp message", {
            ...
            case "notification": {
              toast(String(payload.title || "Notification"), {
                description: String(payload.message || payload.description || ""),
              });
              break;
            }
